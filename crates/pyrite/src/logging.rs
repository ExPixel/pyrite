...
pub fn init(config: &mut SharedConfig) -> anyhow::Result<()> {
    ...
    config.write().logging.reload_handle = Some(builder.reload_handle());
    builder.init();
    Ok(())
}
...
pub fn reload(config: &SharedConfig) -> anyhow::Result<()> {
    ...
    let env_filter = EnvFilter::builder()
        ...
        .parse(log_filters)
        .context("parsing log filter")?;
    if let Some(ref reload_handle) = config.write().logging.reload_handle {
        reload_handle
